...
void MultA(int* C, int* A, int* B, int n) {
	...
	if(n==2) { //base case
		...
		int sqn = n*n;
		cilk_spawn MultA(C,             A,         B,         n/2);
		cilk_spawn MultA(C+(sqn/4),     A,         B+(sqn/4), n/2);
		cilk_spawn MultA(C+((3*sqn)/4), A+(sqn/2), B+(sqn/4), n/2);
		cilk_spawn MultA(C+(sqn/2),     A+(sqn/2), B,         n/2);
		cilk_sync;
		cilk_spawn MultA(C+(sqn/2),     A+((3*sqn)/4), B+(sqn/2),     n/2);
		cilk_spawn MultA(C+((3*sqn)/4), A+((3*sqn)/4), B+((3*sqn)/4), n/2);
		cilk_spawn MultA(C+(sqn/4),     A+(sqn/4),     B+((3*sqn)/4), n/2);
		cilk_spawn MultA(C,             A+(sqn/4),     B+(sqn/2),     n/2);
		cilk_sync;
		return;
	}
}
